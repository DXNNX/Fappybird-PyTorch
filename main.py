...
def preprocess(screen,height,width):
        preprocessed = np.array = cv2.resize(screen, (height,width)) 
...
def main():
    ...
    env = PLE(FlappyBird(), fps=30, display_screen=True, reward_values=reward_values)
    ...
    for episode in range(1, Config.total_episode+1):
        ...
        while not env.game_over():
            ...
            #obs = convert(preprocess(env.getScreenRGB(),84,84))
            obs = convert(env.getScreenGrayscale())
